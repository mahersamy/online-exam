...
export class AuthApiService implements AuthApi{
  ...
  private readonly _verifyCodeApiAdaptorService=inject(VerifyCodeApiAdaptorService);
  ...
  verifyCode(data: VerifyCodeData): Observable<VerfiyCodeResponse> {
    return this._httpClient.post(this._Api_BASE+AuthEndPoint.VERIFYCODE,data).pipe(map((res)=>this._verifyCodeApiAdaptorService.adapt(res))
    ,catchError(
      (error)=>{
        return throwError(() => error);
      }
